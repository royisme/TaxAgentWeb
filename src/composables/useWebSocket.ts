...
import type { ChatMessage, WebSocketPacket,ReceivedAudio } from '@/types/chat'; // Adjust path if needed
...
export function useWebSocket(url: string) {
    ...
    const connect = (): void => {
        ...
        ws.value.onopen = () => {
            ...
            error.value = null;
            messages.value.push({ id: Date.now(), text: 'Connection opened.', type: 'system' });
        };
            ...
            try {
                ...
                if (packet.turn_complete) {
                 ...
                 if (packet.interrupted) {
                    currentAgentMessageId.value = null;
                    messages.value.push({ id: Date.now(), text: '[Agent Interrupted]', type: 'system' });
                    ...
                    packet.content.parts.forEach(part => {
                        ...
                        if (part.text) {
                             ...
                             if (!currentAgentMessageId.value) {
                                 ...
                                 messages.value.push({ id: newId, text: text, type: 'agent', isComplete: packet.turn_complete }); // Mark complete if flag is present
                             } else {
                                 const msgIndex = messages.value.findIndex(m => m.id === currentAgentMessageId.value);
                                 if (msgIndex !== -1) {
                                     ...
                                     messages.value.push({ id: currentAgentMessageId.value, text: text, type: 'agent', isComplete: packet.turn_complete });
                                 }
                             }
                        }
                        // --- Handle Audio Part ---
                ...
                messages.value.push({ id: Date.now(), text: `Error: ${errorMessage}`, type: 'error' });
            }
        };
        ...
        ws.value.onerror = (event: Event) => {
            ...
            messages.value.push({ id: Date.now(), text: 'Connection error.', type: 'error' });
        };
        ...
        ws.value.onclose = (event: CloseEvent) => {
            ...
            ws.value = null;
            currentAgentMessageId.value = null;
            messages.value.push({ id: Date.now(), text: `Connection closed (${event.code}). Attempting to reconnect...`, type: 'system' });
            ...
            if (event.code !== 1000 && event.code !== 1005) {
                reconnectTimeout.value = setTimeout(connect, 5000); // Attempt reconnect
                 ...
                 messages.value.push({ id: Date.now(), text: 'Connection closed normally.', type: 'system' });
            }
        };
    };
    ...
    const sendMessage = (text: string): void => {
        if (ws.value && isConnected.value) {
            messages.value.push({ id: `user-${Date.now()}`, text: text, type: 'user' });
            ...
            console.warn(msg);
            error.value = msg;
            messages.value.push({ id: Date.now(), text: msg, type: 'error' });
        }
    };
